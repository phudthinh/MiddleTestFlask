...
def updateProjectStatus(project_id):
    _user_id = session.get('user')
    if _user_id:
        project = db.session.query(models.Project).filter_by(project_id=project_id, user_id=_user_id).first()
        if project:
            tasks = db.session.query(models.Task).filter_by(project_id=project_id, user_id=_user_id).all()
            if tasks:
...
def search():
    ...
    if _user_id:
        ...
        user = db.session.query(models.User).filter_by(user_id=_user_id).first()
        projects = db.session.query(models.Project).filter(models.Project.name.like('%'+searchProject+'%'), models.Project.user_id == _user_id).all()
        status_id = db.session.query(models.Project).filter(models.Project.status_id.like('%'+searchStatus+'%'), models.Project.user_id == _user_id).all()
